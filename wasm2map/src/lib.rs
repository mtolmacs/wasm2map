...
use error::Error;
use gimli::{Dwarf, EndianSlice, LineProgramHeader, LineRow, RunTimeEndian, Unit};
        ...
        while let Some(header) = iter.next()? {
            ...
            if let Some(program) = unit.line_program.clone() {
                ...
                while let Some((header, row)) = rows.next_row()? {
                    let point = CodePoint::new(&dwarf, &unit, row, header, offset)?;
...
pub struct CodePoint {
    //dwarf: &'l Dwarf<EndianSlice<'l, RunTimeEndian>>,
    path: PathBuf,
    address: i64,
    line: i64,
    column: i64,
...
impl CodePoint {
    /// Create a new line from the DWARF data
    pub fn new(
